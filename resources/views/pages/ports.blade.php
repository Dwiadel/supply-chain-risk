...
@section('content')
<h5 style="color:var(--text-main);margin-bottom:20px;">Port Location Dashboard</h5>
<div class="row g-3">
    <div class="col-md-4">
        <div class="dash-card">
            ...
            <input type="text" id="port-search" placeholder="Nama pelabuhan atau negara..."
                   ...
                   style="background:var(--dark-bg);border:1px solid var(--card-border);color:var(--text-main);">
...
fetch('/api/ports').then(r => r.json()).then(data => {
    ...
    allPorts.forEach(port => {
        L.circleMarker([port.latitude, port.longitude], {
            ...
            fillColor: port.size_category === 'Large' ? '#fd7e14' : '#6c757d',
...
function renderPortList(ports) {
    portListEl.innerHTML = ports.map(p => `
        <div style="padding:8px 0;border-bottom:1px solid var(--card-border);cursor:pointer;"
            ...
            <div style="color:var(--text-main);font-size:13px;font-weight:500;">${p.name}</div>
