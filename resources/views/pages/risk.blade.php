...
<p style="color:var(--text-muted-custom);font-size:13px;margin-bottom:20px;">
    ...
    <code style="background:var(--card-bg);padding:2px 8px;border-radius:4px;color:#0d6efd;">
        Total = (Cuaca×0.30) + (Inflasi×0.25) + (Kurs×0.20) + (Berita×0.25)
    </code>
</p>

<div id="risk-welcome" style="text-align:center;padding:40px 20px;">
    <i class="bi bi-speedometer2" style="font-size:48px;color:#0d6efd;opacity:0.5;display:block;margin-bottom:15px;"></i>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="dash-card" style="text-align:center;">
                ...
                <div id="r-total" style="font-size:80px;font-weight:900;color:#fff;line-height:1;margin:15px 0;">—</div>
        ...
        <div class="col-md-8">
            <div class="dash-card">
                ...
                <div style="margin-top:15px;padding:12px;background:var(--dark-bg);border-radius:8px;">
                    ...
                    <div id="r-formula" style="color:#0d6efd;font-size:12px;font-family:monospace;"></div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="dash-card">
                ...
                <div style="margin-top:15px;display:flex;flex-direction:column;gap:10px;">
                    <div style="padding:12px;background:rgba(25,135,84,0.1);border:1px solid rgba(25,135,84,0.3);border-radius:8px;">
                        <div style="color:#25b574;font-weight:700;font-size:13px;">
                    ...
                    <div style="padding:12px;background:rgba(255,193,7,0.1);border:1px solid rgba(255,193,7,0.3);border-radius:8px;">
                        <div style="color:#ffc107;font-weight:700;font-size:13px;">
                    ...
                    <div style="padding:12px;background:rgba(220,53,69,0.1);border:1px solid rgba(220,53,69,0.3);border-radius:8px;">
                        <div style="color:#ff6b7a;font-weight:700;font-size:13px;">
        ...
        Object.entries(comps).forEach(function(entry) {
            ...
            html += '<div style="margin-bottom:18px;">' +
                '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
                    '<div>' +
                        '<span style="color:#fff;font-weight:600;font-size:13px;">' + c.label + '</span>' +
                ...
                '<div style="display:flex;justify-content:space-between;">' +
                    '<small style="color:var(--text-muted-custom);">Kontribusi ke total: <b style="color:#fff;">' + c.contribution.toFixed(2) + '</b></small>' +
        ...
        gaugeChart = new Chart(document.getElementById('gauge-chart'), {
            ...
            data: {
                datasets: [{
                    ...
                    backgroundColor: [getColor(score), 'rgba(255,255,255,0.05)'],
                    borderWidth: 0,
                    circumference: 180,
                    rotation: 270,
                }]
        ...
        radarChart = new Chart(document.getElementById('radar-chart'), {
            ...
            options: {
                scales: {
                    r: {
                        min: 0, max: 100,
                        ticks: { color: '#8b8fa8', font: { size: 10 }, stepSize: 25 },
                        grid: { color: 'rgba(255,255,255,0.1)' },
                        pointLabels: { color: '#b0b3c8', font: { size: 11 } },
        ...
        contribChart = new Chart(document.getElementById('contrib-chart'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: contributions,
                    backgroundColor: ['rgba(13,110,253,0.7)', 'rgba(37,181,116,0.7)', 'rgba(255,193,7,0.7)', 'rgba(255,107,122,0.7)'],
                    borderWidth: 0,
                    borderRadius: 4,
                }]
            },
        ...
        document.getElementById('r-history-table').innerHTML =
            ...
            data2.data.map(function(r) {
                return '<tr style="border-bottom:1px solid var(--card-border);">' +
                    '<td style="padding:8px;color:#b0b3c8;font-size:12px;">' + new Date(r.calculated_at).toLocaleString('id-ID') + '</td>' +
                    '<td style="padding:8px;text-align:center;font-size:12px;color:#fff;">' + r.weather_score + '</td>' +
                    '<td style="padding:8px;text-align:center;font-size:12px;color:#fff;">' + r.inflation_score + '</td>' +
                    '<td style="padding:8px;text-align:center;font-size:12px;color:#fff;">' + r.currency_score + '</td>' +
                    '<td style="padding:8px;text-align:center;font-size:12px;color:#fff;">' + r.news_sentiment_score + '</td>' +
